...
const AdminMasterDealerEditPage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    fetchMasterDealerDetail();
  }, [id]);

  const fetchMasterDealerDetail = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/master-dealers/${id}`);
      if (res.data?.success) {
      ...
      console.error('Fetch master dealer detail error:', err);
      showToast('Gagal memuat data Master Dealer.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.put(`/master-dealers/${id}`, payload);
      if (res.data?.success) {
        showToast('Data Master Dealer berhasil diperbarui!', 'success');
        setTimeout(() => {
          navigate('/admin/master-data?tab=master-dealers');
        }, 1000);
      }
    } catch (err) {
      console.error('Update master dealer error:', err);
      showToast(err.response?.data?.message || 'Gagal menyimpan perubahan Master Dealer.', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="space-y-3">
        ...
        <nav className="flex items-center gap-1.5 text-xs font-medium text-slate-500">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-blue-600 font-semibold">Edit Master Dealer</span>
        </nav>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-1">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
              Edit Master Dealer: {formData.name || 'Master Dealer'}
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Perbarui informasi supplier laptop baru mitra bisnis inventaris.
            </p>
          </div>
          <Link
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-6 sm:p-7 space-y-6">
            ...
            {/* 4. ALAMAT GUDANG / KANTOR */}
            <div className="space-y-1.5 text-xs">
              <label className="block font-semibold text-slate-700">Alamat Gudang / Kantor</label>
              <textarea
                rows={3}
                maxLength={255}
...
export default AdminMasterDealerEditPage;
