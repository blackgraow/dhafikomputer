  ...
  try {
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS \`laptops\` (
        ...
        \`status\` ENUM('TERSEDIA', 'HABIS', 'DRAFT') DEFAULT 'TERSEDIA',
        \`description\` TEXT NULL,
    ...
    // Ensure panel_type and weight columns exist if table was previously created
    try {
    ...
    if (userRows[0].count === 0) {
      ...
      await conn.query('INSERT INTO users (username, password_hash, name) VALUES (?, ?, ?)', ['admin', hash, 'Admin Dhafi Komputer']);
    }
    ...
    if (brandRows[0].count === 0) {
      await conn.query(`
        INSERT INTO brands (code, name, logo_url) VALUES
        ('BRD-ASUS', 'ASUS', 'https://images.unsplash.com/photo-1611186871348-b1ce696e52c9?w=300'),
        ('BRD-LNV', 'Lenovo', 'https://images.unsplash.com/photo-1588872657578-7efd1f1555ed?w=300'),
        ('BRD-ACR', 'Acer', 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=300'),
        ('BRD-HP', 'HP', 'https://images.unsplash.com/photo-1525547719571-a2d4ac8945e2?w=300'),
        ('BRD-MSI', 'MSI', 'https://images.unsplash.com/photo-1603302576837-37561b2e2302?w=300'),
        ('BRD-DELL', 'Dell', 'https://images.unsplash.com/photo-1593642632823-8f785ba67e45?w=300'),
        ('BRD-AAPL', 'Apple', 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=300');
      `);
    }
    ...
    if (catRows[0].count === 0) {
      await conn.query(`
        ...
        ('CTG-CNS', 'Consumer / Harian', 'Laptop harian, tugas sekolah, dan kerja kantor ringan'),
        ('CTG-GAM', 'Gaming & Performa', 'Laptop gaming dan editing berat'),
        ('CTG-BUS', 'Business / Enterprise', 'Laptop bisnis durabilitas tinggi'),
        ('CTG-ULT', 'Ultrabook & Tipis', 'Laptop tipis dan ringan premium');
      `);
    }

    // Seed default master dealers (MD 1, MD 2, MD 3 - ALL BRAND)
    const [mdRows] = await conn.query('SELECT COUNT(*) as count FROM master_dealers');
    if (mdRows[0].count === 0) {
      await conn.query(`
        INSERT INTO master_dealers (code, name, contact, address, status, notes) VALUES
        ('MD-001', 'MD 1 (Master Dealer Utama)', '081234567890', 'Jalan Master Supplier No. 1, Jakarta', 'AKTIF', 'Supplier Laptop Baru All Brand'),
        ('MD-002', 'MD 2 (Master Dealer Partner)', '081298765432', 'Jalan Master Supplier No. 2, Bandung', 'AKTIF', 'Supplier Laptop Baru All Brand'),
        ('MD-003', 'MD 3 (Master Dealer Regional)', '081311223344', 'Jalan Master Supplier No. 3, Surabaya', 'AKTIF', 'Supplier Laptop Baru All Brand');
      `);
    }

    // Seed default dealers
    const [dlrRows] = await conn.query('SELECT COUNT(*) as count FROM dealers');
    if (dlrRows[0].count === 0) {
      await conn.query(`
        INSERT INTO dealers (code, name, contact, address, status, notes) VALUES
        ('DLR-001', 'Toko Laptop Bandung Computer', '085712341234', 'BEC Lt. 2 Bandung', 'AKTIF', 'Toko Partner Laptop Second'),
        ('DLR-002', 'Cimahi Laptop Center', '085899887766', 'Jl. Raya Cimahi No. 45', 'AKTIF', 'Toko Partner Laptop Second'),
        ('DLR-003', 'Bintang Komputer Trade-in', '081900112233', 'Jl. Merdeka No. 88 Bandung', 'AKTIF', 'Toko Partner Laptop Second');
      `);
    }

    // Seed default laptops if empty, OR update missing specs for existing default laptops in MySQL
    const [laptopRows] = await conn.query('SELECT COUNT(*) as count FROM laptops');
    if (laptopRows[0].count === 0) {
      for (const l of mockDb.laptops) {
        await conn.query(`
          INSERT INTO laptops (
        ...
        `, [
          l.id, l.code, l.name, l.brand_id, l.category_id, l.condition_type, l.source_type,
          l.master_dealer_id, l.dealer_id, l.customer_name, l.customer_contact, l.customer_notes,
          l.processor, l.ram, l.storage, l.gpu, l.screen_size, l.panel_type, l.operating_system, l.color, l.weight, l.release_year,
          l.warranty, l.serial_number, l.condition_notes, l.purchase_price, l.selling_price,
          l.display_stock, l.physical_stock, l.status, l.description, l.primary_image
        ]);
      }
      console.log('✅ Seeded default laptops with complete hardware specifications.');
    } else {
      // Sync/update specs for existing default mock laptops if their processor or other specs are NULL/empty in MySQL
      for (const l of mockDb.laptops) {
        await conn.query(`
          UPDATE laptops SET
            ...
            panel_type = COALESCE(NULLIF(panel_type, ''), ?),
          ...
          WHERE code = ? OR id = ?
        `, [
          l.processor, l.ram, l.storage, l.gpu, l.screen_size, l.panel_type, l.operating_system, l.color, l.weight, l.release_year, l.warranty,
          l.code, l.id
        ]);
      }
    }
